...
public class DisplayGammaCalibration {
    ...
    public static String getCurGamma(int control)  {
        ...
        for (String filePath : GAMMA_FILE_PATH) {
            ...
            values.append(allGammaValues[5+control]).append(" ");
        }
    ...
    public static boolean setGamma(int control, String gamma)  {
        ...
        for (int i = 0; i < 3; i++) {
            String tgtFile = GAMMA_FILE_PATH[i];
            String[] allGammaValues = FileUtils.readOneLine(tgtFile).split(" ");
            ...
            result &= FileUtils.writeLine(tgtFile, TextUtils.join(" ",allGammaValues));
        }
        if (result) {
